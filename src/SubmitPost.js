...
export default function SubmitPost({ posts }) {
  ...
  if (pathname.includes("edit")) {
    postDetails = posts.find((post) => Number(post.id) === Number(id));
  } else {
    postDetails = {};
  }
  const [formValues, setFormValues] = useState(postDetails);
  ...
  return (
    <section className="content-container-custom p-5">
      {updateSuccesfull && <h6>Post update successfull!</h6>}
      {!updateSuccesfull && (
        <>
          <h1 className="fs-3">
            Enter Details for the Location you want to add:
          </h1>
